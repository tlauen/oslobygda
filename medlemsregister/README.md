...
3. **Overføre til Bygdelista:** Når du (eller nokon i styret) er inne i Bygdelista (lokalt), klikk **«Hent nye frå MailerLite»**. Da hentar appen alle abonnentar i gruppa «Medlemmar» og legg til dei som ikkje allereie finst (sjekka på epost). Felt frå MailerLite vert mappa slik: Name → fornamn, Company → mellomnamn, Last name → etternamn, Phone → telefon, State → adresse, City → poststad, Zip → postnummer, Country → medlemstype (Vanleg/Kul). Sett `MAILERLITE_MEDLEMMAR_GROUP_ID=181371641813534665` (eller din gruppe-ID) og `MAILERLITE_API_TOKEN` i miljøet.
